...
let
  ...
  call = {overrides ? {}}: let
    ...
    in
      if (entry.unpack or "file") == "tarball"
      then unpacked.outPath + "/" + name
      else raw.outPath;

    resolveSpec = upLock: spec:
      if isList spec
      then walkPath upLock upLock.root spec
      else spec;

    walkPath = upLock: nodeName: path:
      if path == []
      then nodeName
      else if !(upLock.nodes ? ${nodeName})
      then throw "tack: follows path dead-end: no node '${nodeName}' in flake.lock"
      else let
        key = head path;
    ...
    evalFlake = sourceInfo: flakeDir: upLock: nodeName: levelFollows: deepFollows: let
      ...
      upPins =
        if hasTack
        then fromTOML (readFile tackPinsPath)
        else {};
      ...
      extraArgs =
        if supportsOverrides && tackOverrides != {}
        then {inherit tackOverrides;}
        else {};
    ...
    evalTransitive = upLock: nodeName: sourceInfo: follows:
      evalFlake sourceInfo sourceInfo.outPath upLock nodeName follows follows;

    evalTopFlake = sourceInfo: pin: let
      ...
      upLock =
        if pathExists upLockPath
        then fromJSON (readFile upLockPath)
        else null;
      rootNode =
        if upLock != null
        then upLock.root
    ...
    evalFetch = sourceInfo: pin: subdir: let
    ...
    in
      ...
      if hasTack && tackOverrides != {}
      then let
        upstream = import (path + "/.tack");
      in
        # old resolvers return a plain attrset, not a callable functor
        if upstream ? __functor
        then (upstream {overrides = tackOverrides;}) // {outPath = path;}
        else trace "tack: ${path}: upstream .tack predates override support; overrides will not reach it" path
      else path;

    loadPin = name: pin: let
      pinType = pin.type or (
        if pin.flake or true
        then "flake"
        else "fetch"
      );
      subdir =
