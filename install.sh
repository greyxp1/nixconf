...
# Resolve to the most stable path, then patch the cloned config so that both
# disko and nixos-install (and the copy that lands in /home/grey/nixconf) use
# the disk that was actually selected. Only /tmp/nixconf is modified — the
# upstream repo is untouched.
DEV_FINAL=$(by_id "$DEV")
echo "==> Using device: $DEV_FINAL"
sed -i "s|custom\.disk\.device = \"[^\"]*\"|custom.disk.device = \"$DEV_FINAL\"|" \
  "$WORK_DIR/modules/system/hosts/$HOST/configuration.nix"
